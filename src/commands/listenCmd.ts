...
export async function listenCmd(
  ...
  let frontOfDoubleSidedScanContext: FrontOfDoubleSidedScanContext | null = null;
  while (keepActive) {
    iteration++;
    ...
    try {
      ...
      const r = await processScanWithDestination(destination, selectedScanTarget,
        lastDuplexMode, lastScanTarget,
        folder, tempFolder, scanConfig, deviceCapabilities, scanCount, frontOfDoubleSidedScanContext);
...
async function processScanWithDestination(destination: WalkupDestination,
...
async function setupScanParameters(
  ...
  if (duplexMode == DuplexMode.Duplex) {
    ...
    if (duplexMode == DuplexMode.FrontOfDoubleSided) {
      ...
      scanToPdf = frontOfDoubleSidedScanContext?.scanToPdf ?? isPdf(destination);
