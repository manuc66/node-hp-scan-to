...
const parseString = promisify<string, WalkupScanToCompManifestData>(parser.parseString);
...
export default class WalkupScanToCompManifest {
  ...
  get WalkupScanToCompCapsURI(): string | null {
    const walkupScanToCompCaps = this.data["man:Manifest"]["map:ResourceMap"]["0"]["map:ResourceNode"].find(x => x["map:ResourceType"][0]["wus:WalkupScanToCompResourceType"][0] === "WalkupScanToCompCaps" );
